 ...
 */
class SolrIndexingMiddleware implements MiddlewareInterface
{
    ...
    {
        /** @var IndexingInstructions|null $instructions */
    ...
    protected function handlePageIndexing(
        ...
        try {
            ...
            if ($pageInformation === null || $pageArguments === null || $siteLanguage === null) {
    ...
    protected function indexRecordItem(Item $item, int $language, ServerRequestInterface $request): bool
    {
        ...
        $serverRequest = $request instanceof \TYPO3\CMS\Core\Http\ServerRequest
            ? $request
    ...
    protected function getFullItemRecord(Item $item, int $language): ?array
    {
        ...
        if ($languageField !== null && $l10nParentField !== null) {
            ...
            $site = $item->getSite();
            if ($site !== null && $language > 0 && $language !== -1) {
    ...
    protected function getRecordAccessRootline(Item $item): string
    {
        ...
        if (isset($GLOBALS['TCA'][$item->getType()]['ctrl']['enablecolumns']['fe_group'])) {
            $accessRestriction = $itemRecord[$GLOBALS['TCA'][$item->getType()]['ctrl']['enablecolumns']['fe_group']] ?? '0';
    ...
    protected function getItemTypeConfiguration(Item $item, int $language): array
    {
        ...
        try {
            $solrConfiguration = $frontendEnvironment->getSolrConfigurationFromPageId($pageId, $language, $item->getRootPageUid());
        ...
        $solrConfiguration = $frontendEnvironment->getSolrConfigurationFromPageId($item->getRootPageUid(), $language);
     ...
     * @param Document[] $documents
     */
    protected function addDocumentsToSolr(array $documents, \ApacheSolrForTypo3\Solr\System\Solr\SolrConnection $solrConnection): bool
    {
        if ($documents === []) {
            return true;
        }
