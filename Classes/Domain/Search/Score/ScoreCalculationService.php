...
class ScoreCalculationService
{
    ...
    /**
     * Turns an array of indented lines into a hierarchical array.
     *
     * @param  array   &$lines
     * @param  integer $depth
     * @param  integer $failsafe
     * @return array
     */
    public function parseScores(array &$lines = [], int $depth = 0, $failsafe = 0): array
    {
        if ($failsafe >= 1000) {
            die('failsafe');
        }
    ...
    private function parseLine($line)
    {
        if (preg_match('/(\d+\.\d+) = weight\((.*)\)/', $line, $weightMatch)) {
            ...
            $line = new Score($boost, $field, $score, $searchTerm);
        } elseif (preg_match('/(\d+\.\d+) = sum of:/', $line, $match)) {
            $score = $match[1];
            $line = new Score('', 'sum of', $score, '');
        } elseif (preg_match('/(\d+\.\d+) = max of:/', $line, $match)) {
            $score = $match[1];
            $line = new Score('', 'max of', $score, '');
        } elseif (preg_match('/(\d+\.\d+) = FunctionQuery\((.*)\),/', $line, $match)) {
            $score = $match[1];
            $function = $match[2];
            $line = new Score('', 'boostFunction', $score, $function);
        } elseif (preg_match('/(\d+\.\d+) = (.*)/', $line, $match)) {
            $score = $match[1];
            $misc = $match[2];
            $line = new Score('', '', $score, $misc);
        }
