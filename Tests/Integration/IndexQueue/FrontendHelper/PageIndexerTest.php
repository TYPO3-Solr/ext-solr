...
class PageIndexerTest extends IntegrationTestBase
{
     ...
     *
     *
     *  Note: Indexing a cross-site mount point requires that "index_enable" is set
     *  in the pagetree of the mounted page, as TYPO3 loads the TypoScript
     *  of the mounted page since TYPO3 13.
     */
    #[Test]
    ...
    /**
     * This testcase should check if we can index a cross-site mount, a mount point
     * that mounts a page from another pagetree that has no site configuration
     *
     * There is following scenario:
     *
     *  [0]
    ...
    {
        self::markTestSkipped('Fails as TYPO3 13 currently can\'t mount a page from a pagetree without a site configuration!');
        $GLOBALS['TYPO3_CONF_VARS']['FE']['enable_mount_pids'] = 1;

        $this->cleanUpAllCoresOnSolrServerAndAssertEmpty();
        $this->importCSVDataSet(__DIR__ . '/Fixtures/can_index_mounted_page_outside_site_root.csv');
        $this->addTypoScriptToTemplateRecord(1, 'config.index_enable = 1');
        $this->indexQueuedPage(24, '/en/');
