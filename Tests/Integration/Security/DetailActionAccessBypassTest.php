...
class DetailActionAccessBypassTest extends IntegrationTestBase
{
    ...
    public function detailActionMustNotDiscloseAccessRestrictedDocumentToAnonymousVisitor(): void
    {
        ...
        // Index public (uid 2) + restricted (uid 3) pages. The sub-request pipeline detects
        // the restricted page's fe_group itself, so it reaches Solr with an "access" field
        // that mandates membership in group 1.
        $this->indexPages([2, 3]);
    ...
    {
        $url = sprintf(
            '%s/select?q=*:*&fl=id&wt=json&fq=%s',
            $this->getSolrCoreUrl('core_en'),
