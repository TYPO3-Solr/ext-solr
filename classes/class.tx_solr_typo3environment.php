...
class tx_solr_Typo3Environment implements t3lib_Singleton {
	...
	/**
	 * Checks whether DAM is used, and if so, whether fileadmin is still
	 * enabled.
	 *
	 * @return	boolean	TRUE if DAM is used and fileadmin is enabled, FALSE otherwise.
	 */
	protected function isFileadminEnabledWhileUsingDam() {
		$usingDamWithFileadminEnabled = FALSE;

		if (t3lib_extMgm::isLoaded('dam')) {
			$damConfiguration = unserialize(
				$GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['dam']
			);

			if ($damConfiguration['web_file']) {
				$usingDamWithFileadminEnabled = TRUE;
			}
		}

		return $usingDamWithFileadminEnabled;
	}
	...
	public function isUsingIndexQueue() {
		...
		if ($schedulerLoaded) {
			...
			if (count($indexQueueWorkers)) {
					...
					// scheduler is declared not to be loaded...
			...
			foreach ($indexQueueWorkers as $indexQueueWorker) {
				...
				$solrServer = $indexQueueWorker->getServer();
				if ($solrServer['rootPageUid'] == $rootPage) {
